...
namespace BejeweledLivePlus.Bej3Graphics
{
	public class TimeBonusEffect : Effect
	{
		...
		public override void Update()
		{
			...
			bool flag = GlobalMembersUtils.GetRandFloatU() < (0.12f) * num;
			if (!SexyFramework.GlobalMembers.gIs3D)
			{
				flag = GlobalMembersUtils.GetRandFloatU() < (0.001f) * num;
			}
			if (mElectroBoltVector.Count < Math.Min((3), mTimeBonus * 2 - 1) || flag)
			{
				ElectroBolt electroBolt = new ElectroBolt();
				electroBolt.mHitOtherGem = false;
				electroBolt.mCrossover = !electroBolt.mHitOtherGem && GlobalMembersUtils.GetRandFloatU() < (0.02f);
				if (electroBolt.mHitOtherGem)
				{
					electroBolt.mAngStart = Math.Abs(GlobalMembersUtils.GetRandFloat()) * (float)Math.PI * 2f;
					Piece pieceAtScreenXY = mFXManager.mBoard.GetPieceAtScreenXY((int)(mX + 50f + (float)Math.Cos(electroBolt.mAngStart) * 100f * (0.6f)), (int)(mY + 50f + (float)Math.Sin(electroBolt.mAngStart) * 100f * (0.6f)));
				...
				if (electroBolt.mHitOtherGem)
				{
					electroBolt.mAngEnd = (float)Math.PI + electroBolt.mAngStart + Math.Abs(GlobalMembersUtils.GetRandFloat()) * (0.5f);
					electroBolt.mAngStartD = GlobalMembersUtils.GetRandFloat() * (0.03f);
					electroBolt.mAngEndD = GlobalMembersUtils.GetRandFloat() * (0.03f);
				}
				else if (electroBolt.mCrossover)
				{
					electroBolt.mAngStart = Math.Abs(GlobalMembersUtils.GetRandFloat()) * (float)Math.PI * 2f;
					electroBolt.mAngEnd = electroBolt.mAngStart;
					electroBolt.mAngStartD = GlobalMembersUtils.GetRandFloat() * (0.02f);
					if (electroBolt.mAngStartD < 0f)
					{
						electroBolt.mAngStartD += (-0.02f);
					}
					else
					{
						electroBolt.mAngStartD += (0.02f);
					}
					electroBolt.mAngEndD = 0f - electroBolt.mAngStartD + GlobalMembersUtils.GetRandFloat() * (0.02f);
				}
				else
				{
					electroBolt.mAngStart = Math.Abs(GlobalMembersUtils.GetRandFloat()) * (float)Math.PI * 2f;
					electroBolt.mAngEnd = electroBolt.mAngStart + Math.Abs(GlobalMembersUtils.GetRandFloat()) * (0.5f) + (0.5f);
					electroBolt.mAngStartD = GlobalMembersUtils.GetRandFloat() * (0.0075f);
					electroBolt.mAngEndD = electroBolt.mAngStartD + GlobalMembersUtils.GetRandFloat() * (0.002f);
				}
				electroBolt.mNumMidPoints = 2;
				for (int j = 0; j < electroBolt.mNumMidPoints; j++)
				{
					electroBolt.mMidPointsPos[j] = GlobalMembersUtils.GetRandFloat() * (10f);
					electroBolt.mMidPointsPosD[j] = GlobalMembersUtils.GetRandFloat() * (0.2f);
				}
			...
			for (int k = 0; k < mElectroBoltVector.Count; k++)
			{
				...
				for (int l = 0; l < electroBolt2.mNumMidPoints; l++)
				{
					...
					{
						if (Math.Abs(electroBolt2.mMidPointsPos[l]) >= (float)(25))
						{
							electroBolt2.mMidPointsPosD[l] *= -0.65f;
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.2f))
						{
							electroBolt2.mMidPointsPos[l] = GlobalMembersUtils.GetRandFloat() * (15f);
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.05f))
						{
							electroBolt2.mMidPointsPosD[l] += GlobalMembersUtils.GetRandFloat() * (1.5f);
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.05f))
						{
							electroBolt2.mMidPointsPosD[l] = GlobalMembersUtils.GetRandFloat() * (1.5f);
					...
					else if (electroBolt2.mCrossover)
					{
						...
						else if (GlobalMembersUtils.GetRandFloatU() < (0.1f))
						{
							electroBolt2.mMidPointsPosD[l] += GlobalMembersUtils.GetRandFloat() * (1.5f);
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.1f))
					...
					else
					{
						if (electroBolt2.mMidPointsPos[l] <= 0f)
						{
							...
							electroBolt2.mMidPointsPosD[l] = GlobalMembersUtils.GetRandFloatU() * (0.1f);
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.05f))
						{
							float num2 = ((4f) - electroBolt2.mMidPointsPos[l]) * (0.1f);
							electroBolt2.mMidPointsPosD[l] = num2 + GlobalMembersUtils.GetRandFloat() * (1f);
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.025f))
						{
							electroBolt2.mMidPointsPos[l] = GlobalMembersUtils.GetRandFloatU() * (18f);
						}
						else if (GlobalMembersUtils.GetRandFloatU() < (0.04f))
						{
							electroBolt2.mMidPointsPosD[l] += GlobalMembersUtils.GetRandFloat() * (2.5f);
						}
						if (GlobalMembersUtils.GetRandFloatU() < (0.1f))
						{
							float num3 = 0f;
							float num4 = 0f;
							if (l - 1 >= 0)
						...
						if (GlobalMembersUtils.GetRandFloatU() < (0.2f))
						{
							...
							if (num5 >= 0 && num5 < electroBolt2.mNumMidPoints)
							{
								...
								electroBolt2.mMidPointsPosD[l] += num6 * (0.2f);
							}
						}
						if (GlobalMembersUtils.GetRandFloatU() < (0.1f))
						{
					...
					if (electroBolt2.mMidPointsPos[l] > (12f) || (num <= 0f && GlobalMembersUtils.GetRandFloatU() < (0.01f)))
					{
						flag2 = true;
					}
				}
				if (electroBolt2.mHitOtherGem)
				{
					float num9 = Piece.GetAngleRadius(electroBolt2.mAngStart, mGemColor, theFrame) + (0f);
					...
					if (pieceById != null)
					{
						...
						if (Math.Sqrt(num14 * num14 + num15 * num15) > (double)(70f))
						{
							flag2 = true;
						}
					...
					if (GlobalMembersUtils.GetRandFloatU() < (0.001f))
					{
						flag2 = true;
					}
				}
				...
				else if (GlobalMembersUtils.GetRandFloatU() < (0.005f))
				{
					flag2 = true;
				}
				if (flag2)
			...
			if (BejeweledLivePlus.Misc.Common.Rand() % (30) == 0)
			{
				Effect effect = mFXManager.AllocEffect(Type.TYPE_LIGHT);
				effect.mFlags = 2;
				effect.mX = mX + GlobalMembersUtils.GetRandFloat() * (20f);
				effect.mY = mY + GlobalMembersUtils.GetRandFloat() * (20f);
				effect.mZ = (0.08f);
				effect.mValue[0] = (30f);
				effect.mValue[1] = (-0.1f);
				effect.mAlpha = (0f);
				effect.mDAlpha = (0.1f);
				effect.mScale = (140f);
				effect.mColor = new Color((255), (255), (255));
				if (BejeweledLivePlus.Misc.Common.Rand() % (2) != 0 && mPieceRel != null)
				{
		...
		public override void Draw(Graphics g)
		{
			...
			if (piece != null)
			{
				...
				if (piece.mRotPct == 0f)
				{
					...
					g.SetColor(new Color(255, 255, 255, (int)(Math.Min((float)mElectroBoltVector.Count * (6f) + (float)(64), 255f) * mAlpha)));
			...
			for (int i = 0; i < mElectroBoltVector.Count; i++)
			{
				...
				color.mAlpha = (int)((float)(255) * mAlpha);
				g.SetColor(color);
				float num5 = 0f;
				float angleRadius = Piece.GetAngleRadius(electroBolt.mAngStart, mGemColor, theFrame);
				angleRadius = (float)(((double)mCirclePct * (48.0) + (1.0 - (double)mCirclePct) * (double)angleRadius) * (double)mRadiusScale);
				...
				angleRadius2 = (float)(((double)mCirclePct * (48.0) + (1.0 - (double)mCirclePct) * (double)angleRadius2) * (double)mRadiusScale);
				...
				if (electroBolt.mHitOtherGem)
				{
					...
					if (pieceById != null)
					{
						...
						angleRadius2 = (float)((double)mCirclePct * (48.0) + (1.0 - (double)mCirclePct) * (double)angleRadius2);
				...
				for (int j = 0; j < electroBolt.mNumMidPoints + 1; j++)
				{
					...
					angleRadius3 = (float)(((double)mCirclePct * (48.0) + (1.0 - (double)mCirclePct) * (double)angleRadius3 + (double)num14) * (double)mRadiusScale);
					...
					if (!electroBolt.mCrossover && !electroBolt.mHitOtherGem)
					{
						theColor.mAlpha = (int)Math.Max(2f, 255f * (1f - num5 * (0.03f)));
						theColor2.mAlpha = (int)Math.Max(2f, 255f * (1f - num14 * (0.03f)));
					}
					theColor.mAlpha = (int)((float)theColor.mAlpha * num);
					theColor2.mAlpha = (int)((float)theColor2.mAlpha * num);
					DrawElectroLine(g, GlobalMembersResourcesWP.IMAGE_ELECTROTEX, (float)GlobalMembers.S(num2 + 50) + GlobalMembers.S(num6), (float)GlobalMembers.S(num3 + 50) + GlobalMembers.S(num7), (float)GlobalMembers.S(num2 + 50) + GlobalMembers.S(num15), (float)GlobalMembers.S(num3 + 50) + GlobalMembers.S(num16), electroBolt.mHitOtherGem ? (8f) : (electroBolt.mCrossover ? (9f) : (6f)), theColor, theColor2);
					Color theColor3 = new Color(Color.White);
					Color theColor4 = new Color(Color.White);
					if (!electroBolt.mCrossover && !electroBolt.mHitOtherGem)
					{
						theColor3.mAlpha = (int)Math.Max(2f, 255f * ((0.85f) - num5 * (0.04f)));
						theColor4.mAlpha = (int)Math.Max(2f, 255f * ((0.85f) - num14 * (0.04f)));
					}
					if (electroBolt.mCrossover)
					{
						theColor3.mAlpha = (int)((double)theColor3.mAlpha * (0.5));
						theColor4.mAlpha = (int)((double)theColor4.mAlpha * (0.5));
					}
					theColor3.mAlpha = (int)((float)theColor3.mAlpha * num);
					theColor4.mAlpha = (int)((float)theColor4.mAlpha * num);
					DrawElectroLine(g, GlobalMembersResourcesWP.IMAGE_ELECTROTEX_CENTER, (float)GlobalMembers.S(num2 + 50) + GlobalMembers.S(num6), (float)GlobalMembers.S(num3 + 50) + GlobalMembers.S(num7), (float)GlobalMembers.S(num2 + 50) + GlobalMembers.S(num15), (float)GlobalMembers.S(num3 + 50) + GlobalMembers.S(num16), electroBolt.mHitOtherGem ? (8f) : (electroBolt.mCrossover ? (8f) : (6f)), theColor3, theColor4);
					num6 = num15;
					num7 = num16;
			...
			if (mTimeBonus > 0)
			{
				...
				((ImageFont)g.mFont).PushLayerColor("GLOW", new Color((255), (255), (255), (int)((double)((float)(255) * 0.5f) * (1.0 + (double)(float)Math.Cos((float)mFXManager.mBoard.mUpdateCnt * (0.15f))))));
				g.SetColorizeImages(true);
				...
				if (piece != null && mFXManager.mBoard.GetTicksLeft() <= (500) && mFXManager.mBoard.mGameTicks / (18) % 2 == 0)
				{
