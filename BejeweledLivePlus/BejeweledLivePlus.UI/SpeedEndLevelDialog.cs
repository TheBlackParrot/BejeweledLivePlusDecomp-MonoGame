...
namespace BejeweledLivePlus.UI
{
	public class SpeedEndLevelDialog : EndLevelDialog
	{
		...
		public override void DrawStatsLabels(Graphics g)
		{
			...
			g.WriteString(GlobalMembers._ID("Best Move", 484), (230), (475) + (48), -1, -1);
			g.WriteString(GlobalMembers._ID("Longest Cascade", 485), (230), (475) + (48) * 2, -1, -1);
			g.WriteString(GlobalMembers._ID("Total Time", 486), (230), (475) + (48) * 3, -1, -1);
		}
		...
		public override void DrawStatsText(Graphics g)
		{
			...
			int theY = (475);
			g.WriteString(theString, theX, theY, -1, 1);
			g.WriteString(SexyFramework.Common.CommaSeperate(mGameStats[25]), (750), (475) + (48), -1, 1);
			g.WriteString(SexyFramework.Common.CommaSeperate(mGameStats[24]), (750), (475) + (48) * 2, -1, 1);
			int num = 60 + mTotalBonusTime;
			g.WriteString(string.Format(GlobalMembers._ID("{0}:{1:d2}", 488), num / 60, num % 60), (750), (475) + (48) * 3, -1, 1);
		}
		...
		public override void Draw(Graphics g)
		{
			...
			int integer = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_ICON_FLAME_OFFSET", (-160));
			int integer2 = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_ICON_STAR_OFFSET", (-160));
			int integer3 = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_ICON_HYPERCUBE_OFFSET", (-160));
			int integer4 = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_ICON_LIGHTNING_OFFSET", (-160));
			int integer5 = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_BOX_YELLOW_OFFSET", (-160));
			int integer6 = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_BOX_PINK_OFFSET", (-160));
			int integer7 = SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_BOX_ORANGE_OFFSET", (-160));
			...
			int num = GlobalMembers.S(SexyFramework.GlobalMembers.gSexyApp.GetInteger("LIGHTNING_GAMEOVER_DLG_ICON_LABEL_SPACING", (5)));
			g.SetFont(GlobalMembersResources.FONT_DIALOG);
			((ImageFont)GlobalMembersResources.FONT_DIALOG).PushLayerColor("OUTLINE", new Color((255868960)));
			g.SetColor(new Color((16777215)));
			g.DrawString(string.Format(GlobalMembers._ID("FLAME x{0}", 489), mGameStats[17]), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_ICON_FLAME_ID) + (float)integer) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_ICON_FLAME.GetCelWidth() + num, (723));
			g.DrawString(string.Format(GlobalMembers._ID("STAR x{0}", 490), mGameStats[18]), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_ICON_STAR_ID) + (float)integer2) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_ICON_STAR.GetCelWidth() + num, (723));
			g.DrawString(string.Format(GlobalMembers._ID("HYPER x{0}", 491), mGameStats[19]), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_ICON_HYPERCUBE_ID) + (float)integer3) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_ICON_HYPERCUBE.GetCelWidth() + num, (723));
			g.DrawString(string.Format(GlobalMembers._ID("TIME +{0}:{1:d2}", 492), mTotalBonusTime / 60, mTotalBonusTime % 60), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_ICON_LIGHTNING_ID) + (float)integer4) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_ICON_LIGHTNING.GetCelWidth() + num, (723));
			g.DrawString(GlobalMembers._ID("SPEED", 493), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_BOX_YELLOW_ID) + (float)integer5) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_BOX_YELLOW.GetCelWidth() + num, (723));
			g.DrawString(GlobalMembers._ID("SPECIAL", 494), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_BOX_ORANGE_ID) + (float)integer7) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_BOX_ORANGE.GetCelWidth() + num, (723));
			g.DrawString(GlobalMembers._ID("MATCHES", 495), (int)GlobalMembers.S(GlobalMembersResourcesWP.ImgXOfs(ResourceId.IMAGE_GAMEOVER_BOX_PINK_ID) + (float)integer6) + GlobalMembersResourcesWP.IMAGE_GAMEOVER_BOX_PINK.GetCelWidth() + num, (723));
			...
			int num7 = (360);
			int num8 = (880) / mPointsBreakdown.Count;
			g.SetFont(GlobalMembersResources.FONT_DIALOG);
			Utils.SetFontLayerColor((ImageFont)GlobalMembersResources.FONT_DIALOG, "OUTLINE", new Color(0, 0, 0, 0));
			g.SetColor(new Color((13676688)));
			for (int k = 0; k < 5; k++)
			{
				g.WriteString(string.Format(GlobalMembers._ID("{0}k", 496), (k + 1) * num6 / 1000), (330), (975) + k * (-46), -1, 1);
			}
